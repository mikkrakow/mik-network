...
<div id="content">
  ...
  <?php endif; ?>
	...
	<span class="spacer2"><a href="<?php bloginfo('url'); ?>">&nbsp;<?php bloginfo('name'); ?></a></span><span class="spacer2">&nbsp;wystawy</span>
	</div> <!--close gdzie jestem -->
	
		
		<ul>
        <li>
      <h2 class="widgettitle">Archiwum wystaw</h2>
      <ul>
        <?php query_posts('cat=27&showposts=200'); ?>
		<?php while (have_posts()): the_post(); ?>
		...
		<li>
      <h2 class="widgettitle">Projekty koncepcyjne</h2>
      <ul>
        <?php query_posts('cat=30&showposts=200'); ?>
		<?php while (have_posts()): the_post(); ?>
		<li><a href="<?php the_permalink() ?>"><?php the_title(); ?></a></li>
		<?php endwhile; ?>
      </ul>
